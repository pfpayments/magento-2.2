...
class Info extends \Magento\Payment\Block\Info
{
     ...
     * @var string
     */
    protected $_template = 'PostFinanceCheckout_Payment::payment/method/info.phtml';
    ...
    /**
     * Gets the URL to the transaction detail view in PostFinance Checkout.
     *
     * @return string
     */
    #[\ReturnTypeWillChange]
    ...
    public function getInvoiceDownloadUrl()
    {
        ...
        return $this->$class_backend->getUrl('postfinancecheckout_payment/order/downloadInvoice',
            [
    ...
    {
        $class_backend = $this->isHelperBackend();
        return  $this->$class_backend->getUrl('postfinancecheckout_payment/order/downloadPackingSlip',
            [
                'order_id' => $this->getTransaction()
                    ->getOrderId()
            ]);
    ...
    {
        $class_backend = $this->isHelperBackend();
        return $this->$class_backend->getUrl('postfinancecheckout_payment/order/downloadRefund',
            [
    ...
    public function isRefundDownloadAllowed()
    {
        ...
        if ($this->getTransaction() && $creditmemo != null && $creditmemo->getPostfinancecheckoutExternalId() != null) {
            $storeId = null;
            if ($this->getInfo() instanceof Payment) {
                /** @var Payment $payment */
                $payment = $this->getInfo();
